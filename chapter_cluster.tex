...
\textbf{Cluster cosmology:}\\
Clusters have historical significance in providing crucial information about underlying cosmological model. Dark matter was discovered in Coma cluster in 1930s, when Fritz Zwicky observed that galaxies in cluster were moving too fast to be held by the gravitational pull of the visible galaxies. In the 1990s cluster provided evidence for low matter density universe \cite{white93a}. %Clusters are so large that the ratio of baryon and dark matter content in the cluster should be equal to that of universe as a whole. The gas mass fraction $f_{gas}$ = $\Omega_{b} /\Omega_{m}$; the total baryonic mass can be calculated in rudimentary way by calculating mass of stars and hot gas in the cluster, $\Omega_{b}$ is obtained from big bang nucleosynthesis, one can constrain $\Omega_{m}$ \pending{white}.
\begin{figure}[ht]
...
Accurate cluster mass estimation is essential in order to fully realise the potential of cluster cosmology.
