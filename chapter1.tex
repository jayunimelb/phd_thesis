...
In 1929, Edwin Hubble observed that the galaxies that are farther away from us are moving away at a faster rate. 
This became the first observational evidence of expanding universe. 
...
Hubble's law states that the receding velocity of galaxy is directly proportional to its distance which can be mathematically written as 
\begin{equation}
v = H_{o} d
\end{equation}
...
\end{equation} 
where $h$ is the dimensionless Hubble parameter. Value of `h' as measured by $Planck$ \cite{planck18-6} is 0.674 $\pm$ 0.005.
...
\begin{equation}
 R_{\mu \nu} - \frac{1}{2} g_{\mu \nu} R = 8 \pi GT_{\mu \nu}
\end{equation}
while L.H.S corresponds to the geometry of the universe, R.H.S represents the matter and energy content in the universe.
In the above equation $R_{\mu \nu}$ is the Ricci tensor which depends on the metric and its derivatives, R is the Ricci scalar and $g_{\mu \nu}$ is the metric.  Stress energy tensor, $T_{\mu \nu}$, is given by:
\begin{equation}
T^{\mu}_{\nu} = (\rho + P) u^{\mu} u _{\nu } - P \delta^{\mu}_{\nu}
\end{equation}
...
where $\rho$ = $\Sigma_{i} \rho_{i}$ and P = $\Sigma_{i} P_{i}$ are the sum of energy density and pressure of all the components in the universe respectively.
 Equations ~\ref{Fred1} and ~\ref{Fred2} are the Friedmann equations governing the evolution of the background universe. \\\\  The energy density of the universe just enough to stop the expansion is called critical density. For flat universe $k$  =0, it is given by
...
\dot{\rho}  + H(3\rho + P)  = 0
\end{equation}
Note that the continuity equation holds for individual species as long as we neglect the energy exchange among different components. 
...
Depending on the value of equation of state parameter `w' the components of the universe can be classified as follows:
\begin{itemize}
\item{\textbf{Non relativistic matter}}: For non-relativistic matter such as dark matter and baryons the energy density is equal to the rest mass energy. The pressure is much smaller than the energy-density (P << $\rho$). In such cases we can approximate w = 0, plugging it in the above equation results in $\rho \propto a^{-3}$
...
\item{\textbf{Dark Energy}}: It is a hypothetical form of energy which is responsible for late time cosmic acceleration. While the equation of state parameter depends on the model of Dark Energy and in many of models it is time dependent, here we assume it to be w = -1. Substituting it in the above equation we get $\rho \propto a^{0}$ and hence the cosmic dilution doesn't affect the dark energy density. 
 \end{itemize}
 It is important here to note that the cosmic dilution has different effect on different components. While radiation component is subdominant in today's universe, it played a major role in the early universe. Fig. ~\ref{fig:scaling} shows the energy density of different components as a function of scale factor; as one expects at late times cosmological constant and matter are dominant component, whereas at earlier times radiation played a significant role in the evolution of background universe.
 \begin{figure}[ht]
