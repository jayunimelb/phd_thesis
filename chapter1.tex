...
\begin{equation}
t_{H} = \frac{1}{H_{0}} = 9.836 \:\times \:10^{9}\: h^{-1}\: yr.
\end{equation}
Hubble distance is defined as the distance light travelled in Hubble time which is roughly $3 \:\times \: 10^{3}\: h^{-1}\: Mpc$
...
\end{itemize}
The Friedmann-Lemaître-Robertson-Walker (FLRW) metric is the only metric in General Relativity consistent with the Copernican principle and expanding universe.
...
%Cosmologists measured the luminosity distance of type Ia supernovaes as function of redshift which became the first direct observation evidence of dark energy \cite{knop03}.%to constrain the constituents of cosmology. 
 ...
 \caption{Cosmological distances as a function of redshift; solid lines represent matter dominated universe. Measuring distance as a function of redshift provides valuable information on the constituent of the Universe. }
\label{fig:dist}
 \end{figure} 
...
\begin{equation}
T = T_{0}(1+ \vec{\beta}.\hat{n} + \frac{T(\hat{n})}{T_{0}})
\end{equation}
...
Our universe is only one realisation of the Gaussian random field, so we face fundamental limits on the knowledge available. This is known as cosmic variance.
...
\begin{equation}
\hat{C}^{X}_{l} = \frac{1}{2l +1}\Sigma_{m} |a^{T}_{lm}|^{2}
\end{equation}
...
It was first detected observationally by Degree Angular Scale Interferometer (DASI) in 2002 \citep{kovac02}.
